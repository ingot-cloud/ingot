...
- [x] T0-1：负责人审阅 README / REQUIREMENTS / DESIGN / TASKS / [PLATFORM-API.md](./PLATFORM-API.md)，闭合 D1–D11
  - 依赖：无
  - 验收：DESIGN「待审阅决策记录」表已填决议；change 状态转 `approved`

- [x] T0-2：更新 [security-center-roadmap.md](../../../../docs/requirements/themes/security-center-roadmap.md) L4 为 `in-spec`（本步骤已完成则跳过）
  - 依赖：T0-1 草稿提交
  - 验收：roadmap 表 L4 行 Change 链接指向本目录

- [x] T0-3：更新 [ROADMAP.md](../../../../docs/requirements/ROADMAP.md) `R-2026-026` 状态 `in-spec`
  - 依赖：T0-2
...
- [x] T2-3：E2E 阶段二 remote：Platform 快照 + 失效广播 + 登录路径 429
...
- [x] T3-5：扩展 `SecurityEventType` + 异步上报 ACCESS 事件（需 E2E 验证）
...
- [x] T4-1：Resilience 故障注入：停 security → LKG → 地板 → 恢复 remote
  - 依赖：Phase 1–3
  - 验收：S9 通过；无 fail-open

- [x] T4-2：全量 E2E + L2/L3 回归 + 前端 API 字段抽查（PLATFORM-API vs Swagger）
  - 依赖：T4-1
  - 验收：REQUIREMENTS 验收标准全部勾选

- [x] T4-3：新建 `specs/current/security/access-protection/`（README + SPEC）
  - 依赖：T4-2
  - 验收：current 反映 As-Built；已知限制入 SPEC

- [x] T4-4：更新 roadmap L4 → `done`；归档本 change 至 `specs/changes/archive/2026/`
  - 依赖：T4-3
...
- [x] V1：`ResilientSnapshotFetcher` / `ResilientLoginFailurePolicyLoader` / 滑动窗口单元测试（框架与 adapter 单测已覆盖核心路径）
- [x] V5：[PLATFORM-API.md](./PLATFORM-API.md) 与实现 Controller 字段逐项对照
- [x] V2：migration 011 在 `ingot_security` 执行 + rollback
- [x] V3：Nacos 热刷新 S11（login-failure 阈值 + SDK 开关）
- [x] V4：登录失败 → 403 → ACCESS 事件 DB 直查

---

## 完成检查

- [x] 实现与 DESIGN 一致（含 D1–D11 决议）
- [x] REQUIREMENTS 验收标准全部满足
- [x] Current 已更新（`specs/current/security/access-protection/`）
- [x] [security-center-roadmap.md](../../../../docs/requirements/themes/security-center-roadmap.md) L4 状态与 Change 链接已更新
- [x] [ROADMAP.md](../../../../docs/requirements/ROADMAP.md) R-2026-026 已更新
- [x] Change README 完成记录已填写并归档
