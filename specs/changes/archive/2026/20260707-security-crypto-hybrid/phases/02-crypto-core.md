...
- `CryptoType.HYBRID("hybrid","HYBRID")`；整体模式：`@InCryptoHybridContext` + `@InDecrypt(HYBRID)` + `@InEncrypt(HYBRID)`。
- `InCryptoProperties`：新增内嵌 `Hybrid`（`responseWrap`、`activeKid`、`Map<String,KeyPair> keyPairs`、`Headers headers`、`modeValue`、`publicKeyEndpointEnabled`）。
