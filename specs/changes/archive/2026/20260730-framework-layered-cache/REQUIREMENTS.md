...
- [x] `ingot-framework/ingot-cache` 编译通过并被聚合构建，`settings.gradle` 与 `config/ingot.gradle` 已注册
- [x] 装饰器组合矩阵单测通过：L1/L2/Resilient 各自开关的全部组合下链路完整且语义正确
- [x] 业务规则 1-7 各有对应单测（尤其：LKG 不随 evict 清除、不缓存空值、fail-closed、合法空不降级）
- [x] `VersionedDerivedCache` 单测覆盖 version 回退与 version 相等但 source 不同的场景（规则 9）
- [x] 多 key `evictAll()` 的 Redis SCAN+DEL 行为单测通过
- [x] `.agents/skills/layered-cache/SKILL.md` 可独立指导一次新缓存能力的接入，AGENTS.md 已引用

### Phase 02 gateway

- [x] 冷启动 Feign 调用次数从 4 降为 1（S3）
- [x] `ALL` 失效事件后 Feign 调用次数为 1
- [x] 广播失效丢失时，最迟一个 L1 TTL 周期内节点收敛到最新快照（S2）
- [x] 共享快照 version 未变化时 `GatewayRuleManager.loadRules` 不被调用（规则 10）
- [x] 共享快照 version 变化时（含 TTL 懒刷新路径）Sentinel 规则被 reload（S4）
- [x] `local-floor-enabled=false` 且无 LKG 时仍抛 `PolicyRemoteUnavailableException`，未 fail-open
- [x] Actuator `securitypolicy` 输出字段与迁移前一致
- [x] 上一轮确立的配置正交性未回退：不设 `ingot.security.policy.client.*` 任何键时 remote 模式可装配；单域关闭不影响其他域

### Phase 03 LoginFailure + credential

- [x] L4 change 的 T4-1 与 T4-2 已完成（门禁，2026-08-01 验收归档）
- [x] LoginFailure 失效事件后 L1/L2 被真实清除（修复 D-C）
- [x] credential 迁移前后 Actuator `credentialpolicy` 输出逐字段一致
- [x] credential 的 Redis key `in:credential:configs:all` 与 `in:credential:policy:lkg` 内容格式不变
- [x] credential 配置键 `ingot.security.credential.cache.*` 全部仍生效，默认值不变
- [x] provider 侧 delegate 覆盖（MySQL 直查、不包 Resilient）行为不变

### Phase 04 dict + 收口

- [x] dict 多 key `evict(code)` 与 `evictAll()` 粒度不变
- [x] dict `mode=AUTO|LOCAL|REMOTE|NONE` 四种模式行为不变
- [x] 四处旧实现类已删除，全仓库无残留引用
- [x] 相关模块编译 + 单测通过
- [x] `specs/current/framework/layered-cache/` 已建立并反映 As-Built
