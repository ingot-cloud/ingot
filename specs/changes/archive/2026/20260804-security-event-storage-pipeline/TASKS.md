...
- [x] 实现与 DESIGN 一致，偏差已先回写并重新确认
- [x] REQUIREMENTS 验收标准全部满足
- [x] `specs/current/security/security-event-center/` 已按最终实现更新
- [x] 新增 recording framework current capability（若最终模块边界保持）
- [x] 旧表/旧 mode 的后续破坏性清理已单独建 change 或明确记录
- [x] README 完成记录已填写，Change 状态为 `completed` 并归档
