...
> 状态：completed（V1-V4 验证通过，current 基线已更新，已归档）
...
- LKG 持久快照（Redis 独立命名空间、长存/不过期，仅在远程成功时刷新，含成功空；Redis 为唯一存储，不持进程内副本以保证多节点降级来源一致）与热缓存分离。
...
- D-L（实施期）：LKG **移除进程内二级副本**，Redis 为唯一存储（一致性优先）——避免负载均衡下「Redis LKG 缺失 + 远程宕机」时热/冷节点在 LKG 与 Nacos 地板间分叉。
- D-E：Nacos 地板必须维护**安全基线**（不得为空），否则 D-B 语义下会退回 fail-open。
- D-F：`@ConfigurationProperties` 走 rebinder 自动刷新，**不加 `@RefreshScope`**；`remote` 模式在失效时经 `InvalidationBus` 清热缓存 L1/L2，`local` 模式由 `ConfigurationPropertiesRebinder` 在 Nacos 变更（dataId `in-security-policy.yml`，常量 `NacosConstants.IN_SECURITY_POLICY`，此前文档笔误写作 `in-security-credential.yml`，以代码为准）时重绑定 `CredentialSecurityProperties`。（注：D-K 整改后已移除进程内编译缓存，加载器每次即时编译，无需再单独失效编译缓存。）
...
- 完成日期：代码完成 2026-07-20；V2-V4 运行时验收通过、验收日期 2026-07-24。
- 关联提交或 PR：TBD
- 更新的 current capability：`specs/current/security/credential-security`（已补：remote 弹性降级阶梯 L1/L2/Resilient/LKG/Nacos 地板、来源可观测、初始密码统一抽象与登录期硬拦截、缓存模型整改、LKG 一致性）
- 单元测试（V1，已补并通过，credential 模块）：
  ...
  - `RemoteCredentialPolicyLoaderTest`（`getInitialPasswordConfig` 命中/缺省；`loadPolicies` 排除 `INITIAL_PASSWORD`；T12 追加 `loadPolicies` 随链路变化的「不再永久短路」断言）
...
- 与原设计的差异：
  ...
  - 缓存模型整改（D-K / T12，实施期发现）：移除无 TTL 的进程内编译缓存 `LocalCompiledPolicyCache`，`loadPolicies()` 改为每次经 `getAll()` 即时编译，与 `getInitialPasswordConfig()` 同源同节奏，使弹性阶梯 TTL 有界、可自动恢复；否则 `loadPolicies` 会因编译缓存永久短路而绕过降级阶梯。`local` 模式随之移除 `ApplicationListener<NacosConfigRefreshEvent>`，刷新改由 rebinder 重绑定属性完成（行为等价）。`CredentialCacheCoordinator` 与 provider `CredentialInvalidationPublisher` 去掉编译缓存清理。
  - LKG 一致性整改（D-L / T13，实施期讨论）：`LastKnownGoodStore` 移除进程内 `AtomicReference` 二级副本，Redis 为唯一 LKG 源，`load()` 无 Redis/缺失时返回 `null` → 统一落 Nacos 地板，消除负载均衡下的跨节点分叉。顺带为 `CredentialCacheProperties.l1Ttl/l2Ttl` 加 `@DurationUnit(MINUTES)`，避免无单位配置被按毫秒绑定导致缓存假性未命中。
- 取消原因：—
