...
| 缺陷 | 现状 | 影响 |
|---|---|---|
| **D-A 重复 Feign** | 四域各持独立 `LocalCompiledCache`，但共享的 `ResilientSnapshotFetcher` 自身不缓存（其 Javadoc 明确写「本类不做缓存」） | 冷启动或 `ALL` 失效后最多打出 4 次 Feign，并各写一次 LKG |
...
- **替换 Spring Cache 体系**：[ingot-core](../../../../ingot-framework/ingot-core) 的 `LocalCacheConfig` 与 `ingot-data-redis` 的 `InRedisCacheManager`（`@Cacheable` 用）不在本次范围，两者服务的是服务层 CRUD 缓存，与本框架的「策略快照 + 降级」场景不同。
