...
**D6 单 key 与多 key 统一到 `LayeredCache<K, V>`。** credential / gateway / LoginFailure 是单 key 全量快照，传固定 key（如 `"all"`）；dict 是多 key，传业务 key（code + scope）。多 key 的 `evictAll()` 在 Redis 侧需 SCAN+DEL，沿用 dict 现有实现。dict 作为 Phase 04 的迁移对象，正是为了验证泛型没有被单 key 场景带偏。
