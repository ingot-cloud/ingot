...
| 层次顺序 | Resilient 位于 L1/L2 之下 | DESIGN D3 |
| Sentinel 联动 | 订阅 `CacheRefreshListener` + version 比对；`refresh-interval` 定时兜底默认关闭 | DESIGN D5 |
...
- [ ] T1-6：框架单测
  - 依赖：T1-4、T1-5
  - 验收：装饰器组合矩阵、派生缓存 `(source, version)` 失效、多 key evict、fail-closed、不缓存空值、LKG 不随 evict 清除，全部通过
- [ ] T1-7：skill 与 AGENTS 引用
  - 依赖：T1-6
  - 验收：`.agents/skills/layered-cache/`（`SKILL.md` + `agents/openai.yaml`）可独立指导一次接入；`AGENTS.md` 追加段落说明新增缓存能力默认套用该机制

## Phase 02 · gateway-rule-client 迁移

- [ ] T2-1：共享快照层
  - 依赖：T1-6
  - 验收：`SecurityPolicySnapshotVO` 走 L1+L2+Resilient(LKG+Floor)，替换现有无缓存的 `ResilientSnapshotFetcher`；冷启动与 `ALL` 失效后 Feign 调用次数各为 1（原为最多 4）
- [ ] T2-2：四域派生层
  - 依赖：T2-1
  - 验收：`LocalCompiledCache` 换成 `VersionedDerivedCache`，按共享快照 `(source, version)` 决定是否重编译；version 未变则复用旧编译产物
- [ ] T2-3：Sentinel version 联动
  - 依赖：T2-2
  - 验收：`SentinelGatewayConfiguration.reloadRules` 增加 last-loaded version 比对；订阅 `CacheRefreshListener` 使 TTL 懒刷新也能触发 reload；新增可配 `refresh-interval` 定时兜底（默认关闭）
- [ ] T2-4：正交性回归
  - 依赖：T2-3
  - 验收：不设 `ingot.security.policy.client.*` 任何键时 remote 模式可装配；单域关闭不影响其他域；`local-floor-enabled=false` 且无 LKG 时仍抛 `PolicyRemoteUnavailableException`；Actuator `securitypolicy` 输出与迁移前一致

## Phase 03 · LoginFailure 与 credential 迁移

> **门禁**：[20260729-security-access-protection/TASKS.md](../20260729-security-access-protection/TASKS.md) 的 T4-1、T4-2 必须已完成。

- [ ] T3-1：LoginFailure 迁移
  - 依赖：T2-4 + 外部门禁
  - 验收：换用框架组件并首次补齐 L1+L2；失效事件后 L1/L2 被真实清除（修复 D-C）；同步更新 L4 change 的 DESIGN.md
- [ ] T3-2：credential 迁移
  - 依赖：T3-1
  - 验收：内部实现换框架组件，DESIGN「零行为变化清单」六项逐项核对通过；provider 侧 delegate 覆盖装配行为不变；迁移前后 Actuator 输出与 Redis 内容 diff 为空
...
- [ ] T4-2：清理与全量验证
  - 依赖：T4-1
  - 验收：四处旧实现类删除且全仓库无残留引用；全量编译 + 单测通过；gateway 冷启动 Feign 计数、credential 迁移前后对齐、失效广播端到端三项验证记录在案
- [ ] T4-3：Current 基线
  - 依赖：T4-2
...
- [ ] V1：Phase 01 框架单测全绿（T1-6）
- [ ] V2：Phase 02 gateway 冷启动 Feign 计数由 4 降为 1，且 version 未变时无 Sentinel reload
- [ ] V3：Phase 02 复用 [test-case/security-policy-e2e.md](../../../../test-case/security-policy-e2e.md) 完成 gateway 回归
- [ ] V4：Phase 03 credential 迁移前后 Actuator 与 Redis 内容 diff 为空
- [ ] V5：Phase 04 全量编译 + 单测通过
