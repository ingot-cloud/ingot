...
- 使用者：未登录用户提交账号密码。
- 触发条件：挑战域开启；存在匹配 `POST /bff/auth/login` 的 `ALWAYS` 策略；请求未带有效 `_vc_pass_token`。
- 期望结果：
  - 网关返回 HTTP **412**，`code=CHALLENGE_REQUIRED`。
  - `data` 含 `vcType=image`、`scope=login`、`checkPath=/vc/image/check`、`scopeParam=_vc_scope`、`passTokenParam=_vc_pass_token`、`ttlSec`、`remaining`。
...
- 期望结果：
  - `POST /vc/image/check?_vc_scope=login` 成功后 `data` 含 `_vc_pass_token` 与 `_vc_scope`。
  - `POST /bff/auth/login?_vc_pass_token=<token>` 进入 BFF 登录编排，不再 412。
...
- 期望结果：
  ...
  - **重试必须同时带** `_vc_pass_token` 与 `_vc_scope`（与签发时相同）；消费成功后打 `ATTR_PASS_TOKEN_OK`，**本次跳过 Sentinel**。
...
- 期望结果：
  - 带 `_vc_scope` 的验码接口 **不得** 返回「成功但无 token」（否则客户端会误以为可重试）。
...
13. 查询参数名固定：`_vc_pass_token`、`_vc_scope`（`VCConstants`）。**消费时 scope 取请求 `_vc_scope`**，与签发一致（D10）；禁止用「仅 ALWAYS 策略 scope」或硬编码 `default` 去消费限流挑战 token。
...
17. 拉码 `GET /vc/image`，验码 `POST /vc/image/check`（anji `pointJson` + `token`）。验码成功且带合法 `_vc_scope` 即签发 PassToken，**不再**要求业务请求带 `_vc_code` / `captchaVerification`。
...
- P0：412 闭环、登录种子、remote/local、captcha 与 OTP 拆分、废弃 verifyUrls、Platform 校验、S9 fail-closed、**D10 PassToken scope 消费修复**、E2E。
- P1：`ON_RATE_LIMIT` 地板示例（不强制种子）、Actuator 挑战域来源展示已有则保持。
- P2：挑战记录 / 验码失败拉黑 / SMS·EMAIL 挑战。

## 边界与非目标

- **前端仓库**：本 change 只提供 [PLATFORM-API.md](./PLATFORM-API.md) 契约；登录页必须改为「先 412 → 滑块 → 带 token 重试」。
...
- [ ] A1：无 PassToken 访问 `POST /bff/auth/login` → 412，`data.vcType=image`，`data.scope=login`，`data.checkPath=/vc/image/check`。
- [ ] A2：滑块 check 带 `_vc_scope=login` 返回 `_vc_pass_token`；持该 token **且** `_vc_scope=login` 重试登录进入 BFF，网关不再 412。
- [ ] A3：错误 / 过期 / 错误 scope 的 token 不能放行 ALWAYS 登录。
- [ ] A4：`mode=remote` 时 Platform 新增 ALWAYS 策略（非登录路径）→ 无重启，该路径开始 412；删除或停用后恢复。
- [ ] A5：`mode=local` 时不部署安全中心，仅靠 Nacos 地板完成 A1/A2；改地板策略 `enabled=false` 无重启后登录不再 412。
- [ ] A6：`mode=remote` 停安全中心 → 登录挑战仍按 LKG 或地板执行，不 fail-open。
- [ ] A7：三环境 `ingot.vc.verifyUrls` 为空；登录不要求 `_vc_code`。
- [ ] A8：`challenge.enabled=false` 时登录不 412。
- [ ] A9：白名单 IP 访问登录不 412（须 `blacklist.enabled=true` 且存在 WHITE）。
- [ ] A10：Redis 不可用时，带 `_vc_scope` 的 check **不**返回成功 token；登录仍 412。
- [ ] A11：策略 `challengeType=SMS` 不产生可用的短信挑战闭环（跳过或 412 不指向 `/vc/sms`）。
- [ ] A12：限流命中 + `ON_RATE_LIMIT` 策略 → 412；无该策略 → 429（E2E TC-SP-013/014）。
- [ ] A13：PassToken 签发逻辑不再被注释；`CaptchaVCProcessor` 不再按 `/auth/oauth2/token` 做 grant 特例。
- [ ] A14：D1–D10 已在 DESIGN 闭合，实现与决议一致。
- [ ] A15：[PLATFORM-API.md](./PLATFORM-API.md) 与 Controller / 412 响应体一致。
- [ ] A16：Nacos 动态刷新验证记录在 TASKS：改配置 → 无重启 → 行为变化。
- [ ] A17：仅 `ON_RATE_LIMIT`、策略 `scope` 非 `default`（如 `e2e-anon`）时：412 → check 签发 → 重试同时带 `_vc_pass_token` 与 `_vc_scope` → **200 且跳过 Sentinel**（不再二次 412/429）。缺 `_vc_scope` 或 scope 错误则不能跳过限流。
