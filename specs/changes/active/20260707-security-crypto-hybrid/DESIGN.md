...
- 响应头回带 `X-In-Crypto-Md: h1` 与 `X-In-Crypto-Kv: <activeKid>`（供前端感知密钥轮换并按需刷新公钥）。AAD = 规范化 `md|kv|no|ts`（响应方向以相同约定或省略 nonce）。
...
- `annotation/InCryptoHybrid.java`：组合 `@InDecrypt(HYBRID)+@InEncrypt(HYBRID)`。
- `InCryptoProperties.java`：新增 `hybrid` 配置（`mode`、`responseWrap`、`activeKid`、`keyPairs{kid->{publicKey,privateKey}}`、`headers`、`modeValue`、`publicKeyEndpointEnabled`）。
...
| 步骤 | 行为 | 回滚方式 |
|---|---|---|
| 引入 | 部署新模块与配置，`mode=optional` | 移除注解/关闭配置即回旧行为 |
| 灰度 | 前端逐步接入信封加密，服务端 `optional` 新旧并存 | 前端回退不加密即可 |
| 收敛 | 稳定后切 `strict`，逐步下线旧固定密钥模式 | 切回 `optional` |

- 无数据库变更；密钥集经配置中心下发，轮换通过更新配置触发 `RefreshScopeRefreshedEvent`，`HybridKeyManager` 重新加载密钥快照。
...
- 集成测试：端到端加密请求/响应（POST 与 GET）、`R` 结构解密、多 kid 轮换、`optional`/`strict` 模式、篡改/重放拒绝、公钥端点经网关放行。
- 兼容测试：旧模式与未标记接口回归。
