...
> 状态：2026-09-13 获得实施授权，主 change 为 implementing；当前执行 T01。勾选仅表示满足任务全部验收条件，盘点完成不等于运行时接入完成。未创建前端代码。

## 阶段 1：契约与预检

- [ ] T01 校对资源操作目录、公共 DTO/OpenAPI、DDL 及迁移规则
  - 进度：进行中；已读取全部 change 工件及 PMS current 基线，盘点 38 个控制器的 163 个入口与 28 张仓库源表。已落地 3 个范围/差异枚举和 4 个公共 record，6 项契约测试通过；完整 OpenAPI、ACTION 执行映射及目标 DDL 未完成。D01 已由用户确认并回写契约：独立平台成员及平台组，同一账号可关联多域身份。继续补齐身份契约和目标 DDL。
  - D01 后续：补齐身份/主体/选择器类型及 MemberStatus；目标身份结构 11 张表；累计 13 项 Java 契约、8 项 MySQL 约束测试通过。身份运行时和其他领域结构尚未接入，不勾选 T01/T05。
  - 最新进度：目标结构补至 55 张表（含 9 张保留辅助表），163 个旧入口均记录目标路径/操作/范围执行要求或移除处置；补齐角色、分配、委派、字段等公共类型并导出 21 个 schemas、5 份夹具。22 项 Java 契约、18 项 MySQL 约束测试通过。
  - 剩余：bootstrap、完整资源详情及预览/升级/诊断/审计响应契约，完整端点 OpenAPI 与请求校验；运行时执行器尚未接入，T01 尚未全部验收。
