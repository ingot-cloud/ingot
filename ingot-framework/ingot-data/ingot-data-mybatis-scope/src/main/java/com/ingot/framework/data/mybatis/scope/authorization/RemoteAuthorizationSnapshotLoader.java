...
/**
 * <p>经 PMS 内部接口拉取授权快照；RPC 失败视为远端不可用，不得 fail-open。</p>
 *
 * @author jy
 * @since 1.0.0
 */
...
public class RemoteAuthorizationSnapshotLoader implements AuthorizationSnapshotLoader {
    ...
    private final RemotePmsAuthorizationService remotePmsAuthorizationService;
    ...
    public AuthorizationSnapshotDTO load(long tenantId, long userId) {
        ...
        try {
            R<AuthorizationSnapshotDTO> response = remotePmsAuthorizationService.snapshot(request);
