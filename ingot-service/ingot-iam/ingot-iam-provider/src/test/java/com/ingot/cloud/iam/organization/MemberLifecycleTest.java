...
class MemberLifecycleTest {
    ...
    @Test
    void membershipWritesInvalidateAuthorizationSnapshots() {
        assertEquals("1", service.changeStatus(TENANT, "102", new MemberStatusInput(MemberStatus.SUSPENDED, "0"), 5001, ALLOW));
        assertEquals(1, invalidations.get());
        assertEquals("1", service.changeStatus(TENANT, "102", new MemberStatusInput(MemberStatus.SUSPENDED, "1"), 5002, ALLOW));
        assertEquals(1, invalidations.get());
        var plan = new MemberDepartmentPlan("1", List.of(new MemberDepartmentPlan.Department("11", true),
                new MemberDepartmentPlan.Department("12", false)));
        assertEquals("1", service.replaceDepartments(TENANT, "102", plan, 5003, ALLOW));
        assertEquals(1, invalidations.get());
        var moved = new MemberDepartmentPlan("1", List.of(new MemberDepartmentPlan.Department("13", true)));
        assertEquals("2", service.replaceDepartments(TENANT, "102", moved, 5004, ALLOW));
        assertEquals(2, invalidations.get());
        assertThrows(BizException.class, () -> service.remove(TENANT, "101", new VersionInput("0"), 5005, ALLOW));
        assertEquals(2, invalidations.get());
    }
