...
ingot:
  credential:
    policy:
      # 策略加载模式：local（本地）或 remote（远程）⭐ 新增
      mode: local
...
|-----|----------|------------|
| **数据源** | application.yml | RPC → Credential Service |
| **动态更新** | ❌ 需重启 | ✅ 实时生效 |
| **多租户** | ❌ 不支持 | ✅ 支持 |
| **网络依赖** | ❌ 无依赖 | ⚠️ 需要网络 |
| **配置复杂度** | 🟢 简单 | 🟡 中等 |
| **适用场景** | 开发/测试/简单应用 | 生产环境/多租户系统 |
...
```
1. 应用启动
2. LocalCredentialPolicyLoader 从配置文件读取
3. 创建策略实例
4. 缓存策略
5. PasswordValidator 使用策略校验
```
...
**工作流程：**
```
1. PasswordValidator 调用校验
2. RemoteCredentialPolicyLoader 通过 RPC 获取策略
3. Credential Service 从数据库加载（带缓存）
4. 返回策略列表
5. 执行校验
```
...
// build.gradle
dependencies {
    implementation project(':ingot-credential-api')  // 添加依赖
}
```
...
### Credential Service 内部模式

Credential Service 使用 `DynamicCredentialPolicyLoader`：
...
#### 1. ingot-security-credential（策略引擎）

位置：`ingot-framework/ingot-security-credential`

```
ingot-security-credential/
├── policy/                       # 策略接口和实现
│   ├── PasswordPolicy.java
│   ├── PasswordPolicyUtil.java   # ⭐ 策略工具类（新增）
│   ├── PasswordStrengthPolicy.java
│   ├── PasswordHistoryPolicy.java
│   └── PasswordExpirationPolicy.java
├── validator/                    # 校验器
│   ├── PasswordValidator.java    # 接口
│   └── DefaultPasswordValidator.java  # 默认实现
├── service/                      # 服务接口
│   ├── CredentialPolicyLoader.java    # ⭐ 策略加载器接口（新增）
│   ├── CredentialSecurityService.java # 统一服务接口
│   ├── PasswordHistoryService.java
│   ├── PasswordExpirationService.java
│   └── impl/
│       ├── LocalCredentialPolicyLoader.java   # ⭐ 本地加载器（新增）
│       ├── RemoteCredentialPolicyLoader.java  # ⭐ 远程加载器（新增）
│       ├── DefaultCredentialSecurityService.java
│       ├── NoOpPasswordHistoryService.java
│       └── NoOpPasswordExpirationService.java
├── model/                        # 数据模型
│   ├── PasswordCheckResult.java
│   ├── PolicyCheckContext.java
│   ├── CredentialScene.java      # 场景枚举
│   ├── CredentialPolicyType.java # ⭐ 策略类型枚举（新增）
│   ├── CredentialStatus.java     # ⭐ 凭证状态枚举（新增）
│   ├── request/
...
#### 2. ingot-security-credential-data（数据层实现）⭐ 新增

位置：`ingot-framework/ingot-security-credential-data`

```
ingot-security-credential-data/
├── mapper/                 # MyBatis Mapper
│   ├── PasswordHistoryMapper.java
│   └── PasswordExpirationMapper.java
├── service/                # Service 实现
│   ├── PasswordHistoryServiceImpl.java     # 环形缓冲算法
│   └── PasswordExpirationServiceImpl.java  # 过期检测
└── config/                 # 自动配置
    └── CredentialDataAutoConfiguration.java
```

**职责：** 提供开箱即用的 MyBatis-Plus 数据层实现，零代码集成
...
#### ingot-credential（微服务）

位置：`ingot-service/ingot-credential`

```
ingot-credential/
├── ingot-credential-api/           # RPC 接口
│   ├── model/                      # DTO/VO
│   └── rpc/
│       └── RemoteCredentialService.java
│
└── ingot-credential-provider/      # 服务实现
    ├── service/
    │   ├── DynamicPolicyLoader.java        # 动态策略加载（数据库+本地兜底）
    │   ├── PolicyConfigService.java        # 策略配置管理
    │   └── CredentialValidationService.java
    ├── mapper/
    │   ├── CredentialPolicyConfigMapper.java
    │   └── CredentialAuditLogMapper.java
    └── web/
        ├── PolicyConfigAPI.java            # 策略管理REST API
        └── AuditLogAPI.java                # 审计日志REST API
```

**职责：** 策略配置管理、动态加载、审计日志、RPC服务
...
**统一表名设计：**
所有服务使用相同表名（`password_history`、`password_expiration`），通过 Framework 模块的通用模型直接映射。
