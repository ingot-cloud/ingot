...
3. **现有 `social` 模块的失效逻辑可以平滑迁移**到统一总线，作为后续路线图。
...
| 未来 `SocialBindingChangedEvent` | `social.binding.refresh` | `in:bus:social.binding.refresh` |
| 未来 `PermissionMatrixChangedEvent` | `permission.matrix.refresh` | `in:bus:permission.matrix.refresh` |
...
6. **`RedisMessageListenerContainer` 全应用唯一**：框架 `InRedisMessageConfiguration#redisContainer` 提供唯一一个 `@ConditionalOnMissingBean` 容器。任何模块若需要新增 Redis Pub/Sub 订阅，**必须注入并复用**这个容器，**不要自建第二个容器**——否则一旦遇到同时引入 `ingot-event-bus` 的服务，会在容器注入处出现"required a single bean, but 2 were found"的启动错误。参考 `ingot-social-common` 中 `socialConfigListenerRegistrar` 的做法（注入容器后向其 `addMessageListener`）。
