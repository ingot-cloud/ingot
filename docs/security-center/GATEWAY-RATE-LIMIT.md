...
管理面表字段 `failure_*`、`challenge_failure_limit`、`block_ttl_sec` 保留列兼容，**网关执行面 Phase 1 不读取**；限流违规临时封禁使用 `SentinelBlockHandler` 硬编码阈值（60s 内 30 次 → 封禁 15 分钟）。
...
1. **违规累积**（`ViolationCounter`，60s 滑动窗口，按 IP）：窗口内 ≥ **30** 次 → `TempBlockStore` 封禁 **15** 分钟 + `BlacklistEventReporter` 异步上报 security
...
**跨请求升级**：反复触发 Sentinel 阻断（每次 412 或 429）→ `ViolationCounter` 异步累计 → 60s 窗口内达 30 次 → `TempBlockStore` 写入临时封禁 → **后续请求**在 `BlacklistFilter` 直接 403（非同一次响应内 412 变 403）。
...
详细 yaml 示例见各类 `*Properties` 类 JavaDoc（如 `RateLimitProperties`、`BlacklistProperties`、`ChallengeProperties`）。
...
5. **403 来自 temp**：查 Redis `in:gw:bl:tmp:IP:*`，或等待 15 分钟 TTL；阈值见 `SentinelBlockHandler.DEFAULT_*` 常量。
