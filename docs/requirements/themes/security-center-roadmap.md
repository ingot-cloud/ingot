...
| L5 | 会话安全 | 五 | 在线会话 / 并发会话 / 强制下线统一管理面 | 部分（并发策略可降级，统一管理不可降级） | done | `specs/changes/archive/2026/20260817-security-session-safety`；current `security/session-safety` |
| L6 | 挑战验证 | 六（6.1） | 图形验证码接入登录 / 敏感接口 | 是（触发策略） | done | `specs/changes/archive/2026/20260827-security-challenge-verification`；current `security/challenge-verification` |
